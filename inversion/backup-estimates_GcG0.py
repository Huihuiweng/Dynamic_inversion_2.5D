...
def dynamic_time_warping(slip,STF,X0,T0):
    X    = slip[:,0]
    D    = slip[:,1]
    T    = STF[:,0]
    dM0  = STF[:,1]
    ...
    temp_t   = T[Archoring_T]
    temp_dM0 = dM0[Archoring_T]
    for i in range(Archoring_X,0,-1):
        if(vr[i]==0.0): break
        dt        = grid_size / vr[i]
        temp_t    = temp_t - dt
        RupT[i-1] = temp_t
        for t in range(Archoring_T,1,-1):
            if(temp_t>=T[t-1] and temp_t<T[t]):
                new_dM0 = dM0[t] - (dM0[t]-dM0[t-1])*(T[t]-temp_t)/(T[t]-T[t-1])
                vr[i-1] = new_dM0 / D[i]
                break
    return vr,RupT
...
def Adjust_gc(vr2_0,vr2_1,vr_ite,g0,ave_len,win_old,amp_old,nuc_bin,trapped,Accuracy):
    A           =  1
    gamma       =  1/np.pi
    ...
    if(len(np.where(fitting[win_old::]>Accuracy*vr2_0[win_old::])[0])==0): ## Finish
         return 0.0,fitting.shape[0]-1,0.0

    if(fitting[win_old]<=Accuracy*vr2_0[win_old]):
        win_new  =  win_old + np.where(fitting[win_old::]>Accuracy*vr2_0[win_old::])[0][0]
    ...
    if(win_new==nuc_bin): # For the search of the nucleation zone (<W)
        amp_new = 1 / vr2_0[win_new]
...
#        amp_new  =  1.0
    ...
    if(abs(Delta_gc)/g0[win_new]>0.2):
...
def Adjust_Dtau(slip_0,slip_n1,Dtau,tip,W,mu,Accuracy):
    X         = slip_0[:,0]
    D_0       = slip_0[:,1]
    D_n1      = slip_n1[:,1]
    ...
    for i in range(X.shape[0]):
        if(abs(D_0[i]-D_n1[i])<Accuracy*D_n1[i] or i<tip):
            ...
            Adj_Dtau[i] = np.pi/4.0*mu*(D_0[i]-D_n1[i])/W
...
def set_para(model,iteration,W,Vs,ave_len,lamda,debug):
    ...
    if(iteration==1):
        ...
        ### Retrive the record and read the last simulated results
        ...
        if(not rup_stop):                  # Only update the complete rupture
            Adj_Dtau = Adjust_Dtau(slip_0,slip_1,Dtau_1,bin_p*(win[iteration-2]),W,mu,slip_cutoff)   # Lock the stress of previous faults
        else:
            Adj_Dtau = Dtau_1 
        ...
        if(len(np.where(win[0:iteration-1]==win[iteration-2])[0])<5):
            ...
            if((gc[win[iteration-2],iteration-2]-gc[win[iteration-2],iteration-3])*(gc[win[iteration-2],iteration-3]-gc[win[iteration-2],iteration-4])<0):
                ...
                trapped = False
        delta_gc,win_new,amp_new = Adjust_gc(vr2_0,vr2_1,vr_ite,g0_0,ave_len,win[iteration-2],amp[iteration-2],nuc_bin,trapped,update_cutoff)
        adj_gc                   = np.copy(gc_1)
        if(iteration<=dry_run):
            win_new              = win[iteration-2]
            amp_new              = amp[iteration-2]
        elif(iteration>dry_run and win_new<=nuc_bin):
            adj_gc               = adj_gc + delta_gc
        else:
 ...
 #,'{:.8f}'.format(((adj_gc[i]-gc_1[i])/gc_1[i])/((vr2_1[i]-vr2_0[i])/vr2_0[i])))
                else:
                    print(i,'{:.4f}'.format(vr2_0[i]),'{:.8f}'.format(vr2_1[i]),\
                            '{:.8f}'.format(vr_ite[i]),'{:.8f}'.format((adj_gc[i]-gc_1[i])/1e6),\
                            '{:.4f}'.format((adj_dtau[i]-dtau[i])/adj_dtau[i]))
